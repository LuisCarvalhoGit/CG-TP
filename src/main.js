...
const shieldVisual = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x00ffaa, transparent: true, opacity: 0.4, depthWrite: false, emissive: 0x00ffaa })
);
shieldVisual.visible = false;

const charList = [ { id: 'TIMEKEEPER', name: 'O Cronometrista', price: 0 }, { id: 'JUGGERNAUT', name: 'O Juggernaut', price: 50 }, { id: 'GHOST', name: 'O Fantasma', price: 100 } ];
let currentCharIndex = 0; let menuCharacter = null; 

const mainMenu = document.getElementById('main-menu'); const gameUI = document.getElementById('game-ui'); const gameOverScreen = document.getElementById('game-over-screen'); const btnStart = document.getElementById('btn-start'); const btnChangeChar = document.getElementById('btn-change-char'); const scoreCounter = document.getElementById('score-counter'); const energyCounter = document.getElementById('energy-counter'); const menuHighScore = document.getElementById('menu-high-score'); const bestScoreCounter = document.getElementById('best-score-counter'); const btnEditLight = document.getElementById('btn-edit-light'); const btnExitEdit = document.getElementById('btn-exit-edit'); const btnDevTools = document.getElementById('btn-dev-tools'); const btnPrevChar = document.getElementById('btn-prev-char'); const btnNextChar = document.getElementById('btn-next-char'); const charNameDisplay = document.getElementById('char-name-display'); const uiFps = document.getElementById('ui-fps'); const uiMs = document.getElementById('ui-ms'); const uiMem = document.getElementById('ui-mem'); const menuCoinCounter = document.getElementById('menu-coin-counter'); const gameCoinCounter = document.getElementById('game-coin-counter');
if (menuHighScore) menuHighScore.innerText = highScore; if (menuCoinCounter) menuCoinCounter.innerText = totalCoins;
...
if (btnDevTools) { btnDevTools.addEventListener('click', () => { if (gui._hidden) { gui.show(); playSFX('jump', 0.3); } else { gui.hide(); } btnDevTools.blur(); }); }
if (btnEditLight) { btnEditLight.addEventListener('click', () => { resetEstadoMundo(); gameState = 'STUDIO'; mainMenu.style.display = 'none'; btnExitEdit.style.display = 'block'; gui.show(); if (player && player.mesh) player.mesh.visible = false; if (menuCharacter && menuCharacter.mesh) menuCharacter.mesh.visible = false; controls.enabled = true; camera.position.set(4, 2.5, 1); controls.target.set(0, 0.5, -8); }); }
if (btnExitEdit) { btnExitEdit.addEventListener('click', () => { gameState = 'MENU'; mainMenu.style.display = 'block'; btnExitEdit.style.display = 'none'; gui.hide(); if (player && player.mesh) player.mesh.visible = true; if (menuCharacter && menuCharacter.mesh) menuCharacter.mesh.visible = true; controls.enabled = false; }); }
...
function atualizarUIEnergia() {
    ...
    if (player.abilityReady) { energyCounter.innerText = "PRONTA! (Espaço)" + extra; energyCounter.style.color = "#FFD700"; energyCounter.style.textShadow = "0 0 10px #FFD700"; } 
    else if (player.isAbilityActive) { const timeLeft = Math.max(0, player.abilityTimer).toFixed(1); energyCounter.innerText = `ATIVO! (${timeLeft}s)` + extra; energyCounter.style.color = "#00ffff"; energyCounter.style.textShadow = "0 0 10px #00ffff"; } 
    else { energyCounter.innerText = `${player.jumps}/${player.jumpsToCharge}` + extra; energyCounter.style.color = "white"; energyCounter.style.textShadow = "2px 2px 0 #000"; }
}
...
window.addEventListener('keydown', (event) => {
    ...
    switch(key) { case 'w': case 'arrowup': player.move('up', world); break; case 's': case 'arrowdown': player.move('down', world); break; case 'a': case 'arrowleft': player.move('left', world); break; case 'd': case 'arrowright': player.move('right', world); break; }
...
function animate() {
    ...
    if (hitStopTimer > 0) {
        ...
        if (currentZ > runScore) { 
            runScore = currentZ; if (scoreCounter) scoreCounter.innerText = runScore; 
            if (runScore > highScore) { highScore = runScore; localStorage.setItem('crossyRun_highScore', highScore); if (bestScoreCounter) { bestScoreCounter.innerText = highScore; bestScoreCounter.style.color = "#00ff00"; } if (menuHighScore) menuHighScore.innerText = highScore; }
        }
        ...
        const targetDir = isBlackout ? 0.0 : 1.8; const targetAmb = isBlackout ? 0.05 : 0.3; const targetHem = isBlackout ? 0.05 : 0.6; const targetFog = isBlackout ? new THREE.Color(0x050505) : new THREE.Color(0x6eb8ff);
        directionalLight.intensity = THREE.MathUtils.lerp(directionalLight.intensity, targetDir, delta * 3); ambientLight.intensity = THREE.MathUtils.lerp(ambientLight.intensity, targetAmb, delta * 3); hemisphereLight.intensity = THREE.MathUtils.lerp(hemisphereLight.intensity, targetHem, delta * 3); scene.fog.color.lerp(targetFog, delta * 3); scene.background.lerp(targetFog, delta * 3);
        ...
        world.coins.forEach(c => {
            if (!c.collected && Math.sqrt(Math.pow(px - c.mesh.position.x, 2) + Math.pow(pz - c.laneZ, 2)) < coinRadius) {
                ...
                playSFX('jump', 0.2); particleSystem.spawn(c.mesh.position.x, 0.5, c.laneZ, 'dust', 5); 
            }
        });
        ...
        if (!isGameOver) {
            ...
            for (const car of world.cars) {
                if (Math.abs(pz - car.laneZ) < zTolerance && Math.abs(px - car.mesh.position.x) < (car.width / 2 + 0.3)) {
                    if (player.type === 'JUGGERNAUT' && player.isAbilityActive) {
                        ...
                        particleSystem.spawn(px, 1.0, pz, 'crash', 15); playSFX('crash', 0.5); triggerImpact(0.4, 0.1); 
                    } 
                    else if (activePowerUps.shield > 0) { 
                        activePowerUps.shield = 0; shieldVisual.visible = false; car.speed = 0; car.mesh.position.y += 15 * delta; 
                        particleSystem.spawn(px, 1.0, pz, 'crash', 15); playSFX('crash', 0.6); triggerImpact(0.5, 0.15);
                    }
            ...
            if (!isGameOver) {
                for (const train of world.trains) {
                    if (train.state === 'PASSING' && Math.abs(pz - train.laneZ) < zTolerance && Math.abs(px - train.mesh.position.x) < 18) {
                        if (activePowerUps.shield > 0) { activePowerUps.shield = 0; shieldVisual.visible = false; train.state = 'IDLE'; train.mesh.position.x = 200; particleSystem.spawn(px, 1.0, pz, 'crash', 25); playSFX('crash', 0.8); triggerImpact(0.8, 0.2); }
                        else { isGameOver = true; causeOfDeath = "Esborrachado pelo Expresso!"; break; }
                    }
                }
            }
            if (!isGameOver && !player.isMoving) {
                ...
                if (conveyor && activePowerUps.time <= 0) { 
                    player.mesh.position.x += conveyor.speed * conveyor.direction * delta * 60;
            ...
            if (!isGameOver && world.chasers) {
                for (const chaser of world.chasers) {
                    if (chaser.state !== 'DEAD' && ((px - chaser.mesh.position.x) * (px - chaser.mesh.position.x) + (pz - (chaser.laneZ + chaser.mesh.position.z)) * (pz - (chaser.laneZ + chaser.mesh.position.z))) < 0.45) { 
                        if (player.type === 'JUGGERNAUT' && player.isAbilityActive || activePowerUps.shield > 0) { 
                            chaser.state = 'DEAD'; chaser.mesh.position.y -= 10 * delta; particleSystem.spawn(px, 1.0, pz, 'crash', 15); playSFX('crash', 0.4); triggerImpact(0.4, 0.1);
                            if (activePowerUps.shield > 0) { activePowerUps.shield = 0; shieldVisual.visible = false; }
                        } 
            ...
            if (!isGameOver && currentLaneObj) {
                if ((currentLaneObj.type === 'river' || currentLaneObj.type === 'acid_pit' || currentLaneObj.type === 'abyss_gap') && !player.isMoving) {
                    ...
                    if (laser && laser.isOn) {
                        if (player.type === 'JUGGERNAUT' && player.isAbilityActive) { } 
                        else if (activePowerUps.shield > 0) { activePowerUps.shield = 0; shieldVisual.visible = false; laser.isOn = false; triggerImpact(0.3, 0.1); }
        ...
        if (isGameOver) {
            ...
            else if (causeOfDeath === "Caíste no Abismo!" || causeOfDeath === "O chão desapareceu!") { pType = 'none'; deathSound = 'jump'; } 
            ...
            if (!causeElement) { causeElement = document.createElement('h2'); causeElement.id = 'death-reason-text'; causeElement.style.color = '#FFA500'; causeElement.style.marginTop = '-10px'; causeElement.style.marginBottom = '20px'; causeElement.style.textShadow = '2px 2px 0px #000'; document.querySelector('#game-over-screen div').insertBefore(causeElement, document.querySelector('#game-over-screen p')); }
