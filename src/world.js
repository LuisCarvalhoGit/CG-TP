...
export class World {
    ...
    loadAudioFiles() {
        ...
        ['train', 'jump', 'splash', 'crash', 'powerup'].forEach(name => {
            audioLoader.load(`./sounds/${name}.mp3`, buffer => this.audioBuffers[name] = buffer, undefined, () => {});
        });
    }
    ...
    getElevationAt(z) { return 0; } freezeStaticObject(obj) { obj.updateMatrixWorld(true); obj.matrixAutoUpdate = false; }

    createInitialMap() {
        for (let z = 20; z >= 6; z--) this.createLane(z, 'forest', true, 0, null, 'NONE'); 
        for (let z = 5; z >= 0; z--) this.createLane(z, 'grass', true, 0, null, 'NONE');
        this.lastLaneType = 'grass';
    ...
    createLane(z, type, isSafeZone, depth = 0, prevType = null, eventName = 'NONE', existingGroup = null) {
        ...
        if (type === 'abyss_gap' || type === 'abyss_safe') {
            const pit = new THREE.Mesh(this.geos.lane, this.mats.abyssBlack); pit.position.y = -15; laneGroup.add(pit);
            if (type === 'abyss_safe') { const bridge = new THREE.Mesh(this.geos.lane, this.mats.tunnelGranite); bridge.position.y = -0.5; bridge.receiveShadow = true; laneGroup.add(bridge); }
        } else {
            if (type === 'river' || type === 'acid_pit') { groundGeo = (type === 'acid_pit') ? this.geos.acid : this.geos.river; groundY = -0.65; }
            if (type === 'road') groundMat = this.mats.road; else if (type === 'river') groundMat = this.mats.river; else if (type === 'acid_pit') groundMat = this.mats.acid; else if (type === 'railroad') groundMat = this.mats.railroad; else if (type === 'conveyor') groundMat = this.mats.conveyorBelt; else if (type === 'factory_floor' || type === 'transition_gate' || type === 'laser') groundMat = this.mats.factoryFloor; 
            const ground = new THREE.Mesh(groundGeo, groundMat); ground.position.y = groundY;  
    ...
    addPowerUp(laneGroup, laneZ) {
        ...
        this.powerUps.push({ mesh: puGroup, laneZ: laneZ, type: puType, collected: false, startY: 0.8 });
    }
    ...
    addChaser(laneGroup, type, depth) { const isFactory = this.currentBiome === 'FACTORY'; const chaserGroup = new THREE.Group(); let leftLeg = null, rightLeg = null, leftArm = null, rightArm = null, axeGroup = null; if (isFactory) { const body = new THREE.Mesh(this.geos.droneBody, this.mats.metalMachine); const eye = new THREE.Mesh(this.geos.droneEye, this.mats.droneEye); eye.position.set(0, 0, 0.35); chaserGroup.add(body, eye); chaserGroup.position.y = 1.0; } else { const headGroup = new THREE.Group(); headGroup.position.y = 0.95; const headCube = new THREE.Mesh(this.geos.lumberHead, this.mats.skinMat); const beard = new THREE.Mesh(this.geos.lumberBeard, this.mats.beardMat); beard.position.set(0, -0.1, 0.15); const beardL = new THREE.Mesh(this.geos.lumberBeardSide, this.mats.beardMat); beardL.position.set(-0.2, -0.05, 0); const beardR = new THREE.Mesh(this.geos.lumberBeardSide, this.mats.beardMat); beardR.position.set(0.2, -0.05, 0); const hatRim = new THREE.Mesh(this.geos.lumberHat, this.mats.hatMat); hatRim.position.y = 0.15; const hatTop = new THREE.Mesh(this.geos.lumberHatTop, this.mats.hatMat); hatTop.position.y = 0.3; const eyeL = new THREE.Mesh(this.geos.lumberEye, this.mats.eyeMat); eyeL.position.set(-0.12, 0.05, 0.2); const eyeR = new THREE.Mesh(this.geos.lumberEye, this.mats.eyeMat); eyeR.position.set(0.12, 0.05, 0.2); headGroup.add(headCube, beard, beardL, beardR, hatRim, hatTop, eyeL, eyeR); const torsoGroup = new THREE.Group(); torsoGroup.position.y = 0.5; const shirt = new THREE.Mesh(this.geos.lumberBody, this.mats.shirtMat); const suspL = new THREE.Mesh(this.geos.lumberSuspender, this.mats.suspendersMat); suspL.position.set(-0.15, 0, 0.15); const suspR = new THREE.Mesh(this.geos.lumberSuspender, this.mats.suspendersMat); suspR.position.set(0.15, 0, 0.15); torsoGroup.add(shirt, suspL, suspR); leftArm = new THREE.Group(); leftArm.position.set(-0.35, 0.75, 0); const lArmMesh = new THREE.Mesh(this.geos.lumberArm, this.mats.shirtMat); leftArm.add(lArmMesh); rightArm = new THREE.Group(); rightArm.position.set(0.35, 0.75, 0); const rArmMesh = new THREE.Mesh(this.geos.lumberArm, this.mats.shirtMat); rightArm.add(rArmMesh); axeGroup = new THREE.Group(); axeGroup.position.set(0, -0.5, 0.1); axeGroup.rotation.x = -Math.PI/2; const handle = new THREE.Mesh(this.geos.axeHandle, this.mats.axeHandleMat); const axeHeadGroup = new THREE.Group(); axeHeadGroup.position.y = 0.35; const ironHead = new THREE.Mesh(this.geos.axeHead, this.mats.metalMachine); const ironBlade = new THREE.Mesh(this.geos.axeBlade, this.mats.axeIronMat); ironBlade.position.z = 0.2; axeHeadGroup.add(ironHead, ironBlade); axeGroup.add(handle, axeHeadGroup); rightArm.add(axeGroup); leftLeg = new THREE.Mesh(this.geos.lumberLeg, this.mats.pantsMat); leftLeg.position.set(-0.15, 0.25, 0); rightLeg = new THREE.Mesh(this.geos.lumberLeg, this.mats.pantsMat); rightLeg.position.set(0.15, 0.25, 0); chaserGroup.add(headGroup, torsoGroup, leftArm, rightArm, leftLeg, rightLeg); chaserGroup.position.y = 0.25; } chaserGroup.position.x = (Math.random() - 0.5) * 20; chaserGroup.castShadow = true; laneGroup.add(chaserGroup); this.chasers.push({ mesh: chaserGroup, laneZ: laneGroup.position.z, state: 'PATROL', patrolDir: Math.random() > 0.5 ? 1 : -1, speed: 0.015 + (depth * 0.0002), chaseTimer: 0, isFactory: isFactory, leftLeg: leftLeg, rightLeg: rightLeg, leftArm: leftArm, rightArm: rightArm, axe: axeGroup }); }
    ...
    warmupShaders(renderer, camera) { const warmupGroup = new THREE.Group(); this.scene.add(warmupGroup); const sampleTypes = ['road', 'river', 'railroad', 'conveyor', 'acid_pit', 'factory_floor', 'grass']; sampleTypes.forEach((type, i) => { const tempLane = new THREE.Group(); const ground = new THREE.Mesh(this.geos.lane, this.mats.grass); tempLane.add(ground); warmupGroup.add(tempLane); }); warmupGroup.traverse((child) => { if (child.isMesh) child.frustumCulled = false; }); renderer.compile(this.scene, camera); renderer.render(this.scene, camera); this.scene.remove(warmupGroup); }
}