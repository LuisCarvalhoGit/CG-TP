...
class ParticleSystem {
    ...
    spawn(x, y, z, type, count) {
        for(let i = 0; i < count; i++) {
            ...
            else if(type === 'laser') mat = Math.random() > 0.5 ? this.mats.blood : new THREE.MeshBasicMaterial({color: 0xff0000}); // Fogo do Laser
            ...
            if(type === 'dust') { vx *= 0.3; vy = Math.random() * 2 + 1; vz *= 0.3; startScale = 0.5; } 
    ...
    update(delta) {
        for(let i = this.particles.length - 1; i >= 0; i--) {
            ...
            else { p.vy -= 18 * delta; p.mesh.position.x += p.vx * delta; p.mesh.position.y += p.vy * delta; p.mesh.position.z += p.vz * delta; p.mesh.rotation.x += p.vx * delta; p.mesh.rotation.y += p.vy * delta; p.mesh.scale.setScalar(Math.max(0, p.life * p.baseScale)); }
        }
    }
...
directionalLight.position.set(15, 25, -15); directionalLight.castShadow = true; directionalLight.shadow.mapSize.width = 2048; directionalLight.shadow.mapSize.height = 2048; directionalLight.shadow.camera.near = 0.5; directionalLight.shadow.camera.far = 60; directionalLight.shadow.camera.left = -20; directionalLight.shadow.camera.right = 20; directionalLight.shadow.camera.top = 20; directionalLight.shadow.camera.bottom = -20; directionalLight.shadow.bias = -0.001; 
...
gui.domElement.style.position = 'absolute'; gui.domElement.style.top = '100px'; gui.domElement.style.right = '15px'; gui.domElement.style.zIndex = '10000'; gui.domElement.addEventListener('keydown', (e) => e.stopPropagation());

const luzAmbiente = gui.addFolder('Luz Ambiente');
luzAmbiente.addColor({ cor: ambientLight.color.getHex() }, 'cor').onChange(v => ambientLight.color.setHex(v)).name('Cor Base'); luzAmbiente.add(ambientLight, 'intensity', 0, 2).name('Intensidade');
const luzDirecional = gui.addFolder('Luz Direcional (Sol)');
luzDirecional.addColor({ cor: directionalLight.color.getHex() }, 'cor').onChange(v => directionalLight.color.setHex(v)).name('Cor do Sol'); luzDirecional.add(directionalLight, 'intensity', 0, 5).name('Intensidade'); luzDirecional.add(directionalLight.position, 'x', -50, 50).name('Posição X'); luzDirecional.add(directionalLight.position, 'y', 0, 50).name('Posição Y'); luzDirecional.add(directionalLight.position, 'z', -50, 50).name('Posição Z');
const luzHemisferica = gui.addFolder('Luz Hemisférica');
luzHemisferica.addColor({ ceu: hemisphereLight.color.getHex() }, 'ceu').onChange(v => hemisphereLight.color.setHex(v)).name('Cor do Céu'); luzHemisferica.addColor({ chao: hemisphereLight.groundColor.getHex() }, 'chao').onChange(v => hemisphereLight.groundColor.setHex(v)).name('Cor do Chão'); luzHemisferica.add(hemisphereLight, 'intensity', 0, 2).name('Intensidade');
const efeitoBloom = gui.addFolder('Efeito Neon (Bloom)');
efeitoBloom.add(bloomPass, 'threshold', 0.0, 1.0).name('Limiar de Brilho').onChange(v => bloomPass.threshold = Number(v)); efeitoBloom.add(bloomPass, 'strength', 0.0, 3.0).name('Intensidade').onChange(v => bloomPass.strength = Number(v)); efeitoBloom.add(bloomPass, 'radius', 0.0, 1.0).name('Raio').onChange(v => bloomPass.radius = Number(v));

luzAmbiente.close(); luzDirecional.close(); luzHemisferica.close(); efeitoBloom.close(); gui.hide(); 
...
const stormMat = new THREE.MeshStandardMaterial({ color: 0xff0000, transparent: true, opacity: 0.6, emissive: 0xff0000, emissiveIntensity: 3.0, depthWrite: false, side: THREE.DoubleSide });
const stormWall = new THREE.Mesh(stormGeo, stormMat);
stormWall.position.set(0, 1.0, 10); stormGroup.add(stormWall); scene.add(stormGroup);
...
let player = null; let gameState = 'MENU'; const clock = new THREE.Clock(); let runScore = 0; 
...
let deathLineZ = 5; const cameraTarget = new THREE.Vector3(0, 0, 0); let deathTimer = 0;
...
let currentCharIndex = 0; let menuCharacter = null; 

const mainMenu = document.getElementById('main-menu'); const gameUI = document.getElementById('game-ui'); const gameOverScreen = document.getElementById('game-over-screen'); const btnStart = document.getElementById('btn-start'); const btnChangeChar = document.getElementById('btn-change-char'); const scoreCounter = document.getElementById('score-counter'); const energyCounter = document.getElementById('energy-counter'); const menuHighScore = document.getElementById('menu-high-score'); const bestScoreCounter = document.getElementById('best-score-counter'); const btnEditLight = document.getElementById('btn-edit-light'); const btnExitEdit = document.getElementById('btn-exit-edit'); const btnDevTools = document.getElementById('btn-dev-tools');
const btnPrevChar = document.getElementById('btn-prev-char'); const btnNextChar = document.getElementById('btn-next-char'); const charNameDisplay = document.getElementById('char-name-display');
const uiFps = document.getElementById('ui-fps'); const uiMs = document.getElementById('ui-ms'); const uiMem = document.getElementById('ui-mem');
const menuCoinCounter = document.getElementById('menu-coin-counter'); const gameCoinCounter = document.getElementById('game-coin-counter');

if (menuHighScore) menuHighScore.innerText = highScore;
if (menuCoinCounter) menuCoinCounter.innerText = totalCoins;

let framesContados = 0; let ultimoTempoMecanica = performance.now();
...
function updateShowcase() {
    ...
    if (unlockedChars.includes(charInfo.id)) { btnStart.innerText = "INICIAR JOGO"; btnStart.style.background = "#FFD700"; } 
    else { btnStart.innerText = `COMPRAR (${charInfo.price} MOEDAS)`; btnStart.style.background = totalCoins >= charInfo.price ? "#00ff00" : "#555555"; }
}
...
cheatFolder.add(devCheats, 'adicionarMoedas').name('💰 +1000 Moedas'); cheatFolder.add(devCheats, 'zerarMoedas').name('💸 Ficar Pobre (0)'); cheatFolder.add(devCheats, 'desbloquearTudo').name('🔓 Desbloquear Todos'); cheatFolder.add(devCheats, 'bloquearTudo').name('🔒 Bloquear Heróis'); cheatFolder.close(); 
...
if (btnDevTools) {
    btnDevTools.addEventListener('click', () => {
        if (gui._hidden) { gui.show(); playSFX('jump', 0.3); } else { gui.hide(); }
        btnDevTools.blur();
    });
}
...
if (btnEditLight) {
    btnEditLight.addEventListener('click', () => {
        resetEstadoMundo(); gameState = 'STUDIO'; mainMenu.style.display = 'none'; btnExitEdit.style.display = 'block'; gui.show(); 
        if (player && player.mesh) player.mesh.visible = false;
        if (menuCharacter && menuCharacter.mesh) menuCharacter.mesh.visible = false; 
        controls.enabled = true; camera.position.set(4, 2.5, 1); controls.target.set(0, 0.5, -8); 
    });
}

if (btnExitEdit) {
    btnExitEdit.addEventListener('click', () => {
        gameState = 'MENU'; mainMenu.style.display = 'block'; btnExitEdit.style.display = 'none'; gui.hide(); 
...
function iniciarJogo() {
    ...
    gameState = 'PLAYING'; mainMenu.style.display = 'none'; gameOverScreen.style.display = 'none'; gameUI.style.display = 'block';  
...
if (btnStart) {
    btnStart.addEventListener('click', () => {
        const charInfo = charList[currentCharIndex];
        if (unlockedChars.includes(charInfo.id)) { iniciarJogo(); } 
        else {
            if (totalCoins >= charInfo.price) {
                totalCoins -= charInfo.price; unlockedChars.push(charInfo.id); 
                localStorage.setItem('crossyRun_coins', totalCoins); localStorage.setItem('crossyRun_unlocked', JSON.stringify(unlockedChars));
                if (menuCoinCounter) menuCoinCounter.innerText = totalCoins;
                playSFX('powerup', 0.8); particleSystem.spawn(2.5, -0.5, -5, 'dust', 30); updateShowcase(); 
            } else {
                btnStart.innerText = "MOEDAS INSUFICIENTES!"; setTimeout(() => updateShowcase(), 1000); 
            }
        }
    });
}
...
if (btnChangeChar) {
    btnChangeChar.addEventListener('click', () => {
        resetEstadoMundo(); gameOverScreen.style.display = 'none'; mainMenu.style.display = 'block'; gameState = 'MENU';
        if (menuCharacter && menuCharacter.mesh) menuCharacter.mesh.visible = true; 
        btnChangeChar.blur();
    });
}
...
function atualizarUIEnergia() {
    ...
    if (player.abilityReady) { energyCounter.innerText = "PRONTA! (Espaço)"; energyCounter.style.color = "#FFD700"; energyCounter.style.textShadow = "0 0 10px #FFD700"; } 
    else if (player.isAbilityActive) { const timeLeft = Math.max(0, player.abilityTimer).toFixed(1); energyCounter.innerText = `ATIVO! (${timeLeft}s)`; energyCounter.style.color = "#00ffff"; energyCounter.style.textShadow = "0 0 10px #00ffff"; } 
    else { energyCounter.innerText = `${player.jumps}/${player.jumpsToCharge}`; energyCounter.style.color = "white"; energyCounter.style.textShadow = "2px 2px 0 #000"; }
}

let cameraMode = 'isometric'; let transitionProgress = 1; const transitionSpeed = 0.03;
...
window.addEventListener('keydown', (event) => {
    ...
    if (key === ' ' || event.code === 'Space') { 
        ...
        if (player.abilityReady) { player.activateAbility(); atualizarUIEnergia(); playSFX('powerup', 0.6); }
    }
    ...
    if (!wasMoving && player.isMoving) { playSFX('jump', 0.2); particleSystem.spawn(player.mesh.position.x, 0.2, player.mesh.position.z, 'dust', 5); }
    atualizarUIEnergia();
});
...
function animate() {
    ...
    if (agora - ultimoTempoMecanica >= 1000) {
        if (uiFps) { uiFps.innerText = framesContados; uiFps.style.color = framesContados >= 55 ? '#00ff00' : (framesContados >= 40 ? '#ffa500' : '#ff0000'); }
        if (uiMem && performance.memory) { uiMem.innerText = Math.round(performance.memory.usedJSHeapSize / 1048576) + " MB"; } 
    ...
    if (gameState === 'PLAYING' && player) {
        player.update(delta);
        if (player.isAbilityActive || (!player.isAbilityActive && !player.abilityReady)) atualizarUIEnergia();
        ...
        if (currentZ > runScore) { 
            ...
            if (runScore > highScore) {
                highScore = runScore; localStorage.setItem('crossyRun_highScore', highScore);
                if (bestScoreCounter) { bestScoreCounter.innerText = highScore; bestScoreCounter.style.color = "#00ff00"; }
                if (menuHighScore) menuHighScore.innerText = highScore;
            }
        }

        // --- APAGÃO (CONTROLO GLOBAL DA LUZ) ---
        ...
        const targetFog = isBlackout ? new THREE.Color(0x050505) : new THREE.Color(0x6eb8ff);

        directionalLight.intensity = THREE.MathUtils.lerp(directionalLight.intensity, targetDir, delta * 3);
        ambientLight.intensity = THREE.MathUtils.lerp(ambientLight.intensity, targetAmb, delta * 3);
        hemisphereLight.intensity = THREE.MathUtils.lerp(hemisphereLight.intensity, targetHem, delta * 3);
        ...
        const currentOffset = { x: THREE.MathUtils.lerp(config.topDown.x, config.isometric.x, transitionProgress), y: THREE.MathUtils.lerp(config.topDown.y, config.isometric.y, transitionProgress), z: THREE.MathUtils.lerp(config.topDown.z, config.isometric.z, transitionProgress) };
        ...
        camera.position.lerp(idealCamPos, 4.0 * delta); camera.lookAt(cameraTarget);

        world.updateMap(player.mesh.position.z);
        let worldDelta = delta; if (player.type === 'TIMEKEEPER' && player.isAbilityActive) worldDelta *= 0.15; 
        ...
        world.coins.forEach(c => {
            if (!c.collected && Math.sqrt(Math.pow(px - c.mesh.position.x, 2) + Math.pow(pz - c.laneZ, 2)) < 0.8) {
                c.collected = true; c.mesh.visible = false; totalCoins++;
                localStorage.setItem('crossyRun_coins', totalCoins);
                if (gameCoinCounter) gameCoinCounter.innerText = totalCoins; if (menuCoinCounter) menuCoinCounter.innerText = totalCoins;
                playSFX('powerup', 0.3); particleSystem.spawn(c.mesh.position.x, 0.5, c.laneZ, 'dust', 10); 
            }
        });
        ...
        if (!isGameOver) {
            ...
            for (const car of world.cars) {
                if (Math.abs(pz - car.laneZ) < zTolerance && Math.abs(px - car.mesh.position.x) < (car.width / 2 + 0.3)) {
                    if (player.type === 'JUGGERNAUT' && player.isAbilityActive) {
                        ...
                        particleSystem.spawn(px, 1.0, pz, 'crash', 10); playSFX('crash', 0.4);
            ...
            if (!isGameOver && world.chasers) {
                for (const chaser of world.chasers) {
                    if (chaser.state !== 'DEAD' && ((px - chaser.mesh.position.x) * (px - chaser.mesh.position.x) + (pz - (chaser.laneZ + chaser.mesh.position.z)) * (pz - (chaser.laneZ + chaser.mesh.position.z))) < 0.45) { 
                        if (player.type === 'JUGGERNAUT' && player.isAbilityActive) { chaser.state = 'DEAD'; chaser.mesh.position.y -= 10 * delta; particleSystem.spawn(px, 1.0, pz, 'crash', 10); playSFX('crash', 0.4); } 
                        else { isGameOver = true; causeOfDeath = chaser.isFactory ? "Desintegrado pelo Drone!" : "Levaste uma machadada do Lenhador!"; break; }
                    }
                }
            }
            if (!isGameOver && currentLaneObj) {
                // Água / Ácido / ABISMO (Plataformas)
                if ((currentLaneObj.type === 'river' || currentLaneObj.type === 'acid_pit' || currentLaneObj.type === 'abyss_gap') && !player.isMoving) {
                    ...
                    for (const log of world.logs) {
                        if (log.laneZ === currentLaneZ && Math.abs(px - log.mesh.position.x) < (log.width / 2 + 0.1)) {
                            onPlatform = true; player.mesh.position.x += log.speed * log.direction * worldDelta * 60;
                ...
                // LASER EVENT
                    ...
                    if (laser && laser.isOn) {
                        if (player.type === 'JUGGERNAUT' && player.isAbilityActive) { /* Imune */ } 
                        else { isGameOver = true; causeOfDeath = "Frito pelos Lasers!"; }
                    }
                }
            }
        }
        ...
        if (isGameOver) {
            ...
            gameState = 'DYING'; deathTimer = 1.5; 
            let pType = 'crash'; let deathSound = 'crash';
            ...
            else if (causeOfDeath === "Frito pelos Lasers!") { pType = 'laser'; deathSound = 'crash'; }
            else if (causeOfDeath === "Caíste no Abismo!") { pType = 'none'; deathSound = 'jump'; } // Abismo não tem partículas, só cais.
            
            if (pType !== 'none') particleSystem.spawn(player.mesh.position.x, player.mesh.position.y, player.mesh.position.z, pType, 30);
            playSFX(deathSound, 0.7);

            let causeElement = document.getElementById('death-reason-text');
            if (!causeElement) {
                causeElement = document.createElement('h2'); causeElement.id = 'death-reason-text'; causeElement.style.color = '#FFA500'; causeElement.style.marginTop = '-10px'; causeElement.style.marginBottom = '20px'; causeElement.style.textShadow = '2px 2px 0px #000';
        ...
        player.update(delta); world.update(delta * 0.2, player.mesh.position); deathTimer -= delta;
        
        // Efeito de queda infinita se a morte foi no Abismo
        const causeElement = document.getElementById('death-reason-text');
        if (causeElement && causeElement.innerText === "Caíste no Abismo!") {
        ...
        // Restabelece a luz normal no menu
        directionalLight.intensity = THREE.MathUtils.lerp(directionalLight.intensity, 1.8, delta * 3);
        ambientLight.intensity = THREE.MathUtils.lerp(ambientLight.intensity, 0.3, delta * 3);
        hemisphereLight.intensity = THREE.MathUtils.lerp(hemisphereLight.intensity, 0.6, delta * 3);
        scene.fog.color.lerp(new THREE.Color(0x6eb8ff), delta * 3);
        scene.background.lerp(new THREE.Color(0x6eb8ff), delta * 3);
    }
